<template>
    ...
    <div>
        ...
        <div class="container">
            <editor @editorMounted="initBlockContent" @editorUpdated="updateBlog" :isSaving="isSaving" ref="editor"/>
        </div>
        </div>
    </div>
</template>
...
export default {
    ...
    methods: {
        ...
        updateBlogStatus({ closeModal }, status) {
            ...
            const message = status === 'published' ? 'Blog has been successfully published' : 'Blog has been successfully un-published';
            ...
            this.$store.dispatch('instructor/blog/updateBlog', { data: blogContent, id: this.blog._id })
                        ...
                        .catch(_ => this.$toasted.error('Blog cannot be published', { duration: 3000 }))
        },
        ...
        //     this.$store.dispatch('instructor/blog/updateBlog', { data: blogContent, id: this.blog._id })
        //                 .then(_ => {
        //                     this.$toasted.success('Blog has been successfully unpublished', { duration: 3000 });
        //                     closeModal();
        //                     }) 
        //                 .catch(_ => this.$toasted.error('Blog cannot be unpublished', { duration: 3000 }))
        // },
        checkBlogValidity() {
